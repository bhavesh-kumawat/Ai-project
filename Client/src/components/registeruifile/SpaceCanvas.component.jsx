...
function SpaceCanvas() {
  ...
  useEffect(() => {
    ...
    const spawnMeteor = (m) => {
      m.x = Math.random() * canvas.width * 0.65;
      m.y = Math.random() * canvas.height * 0.38;
    ...
    const draw = () => {
      ...
      for (const s of stars) {
        const op = s.base + Math.sin(t * s.speed * 60 + s.phase) * 0.22;
        ctx.beginPath();
      ...
      for (const m of meteors) {
        ...
        const dx = m.len * cos;
        const dy = m.len * sin;
        const tx = m.x + dx * m.prog;
        const ty = m.y + dy * m.prog;
